...
class Connect4NNet(nn.Module):
    def __init__(self, game, args):
        ...
        self.status_layer = nn.Linear(self.board_x * self.board_y * 2, self.board_x * self.board_y) #should interpret threat/score map and provide a signal about the current state of the game
    ...
    def forward(self, s):
        ...
        s = F.dropout(self.analysis_layer_bn(F.leaky_relu(self.analysis_layer(s))), p=dropout)

        # action layers
        pi = F.dropout(self.interpretation_layer_bn(F.leaky_relu(self.interpretation_layer(s))), p=dropout)
